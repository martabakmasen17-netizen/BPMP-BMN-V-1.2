...
async function main() {
  ...
  try {
    ...
    if (!syncRes.ok) {
       console.error("Failed to fetch existing data:", await syncRes.text());
       return;
    }
    
    let db = await syncRes.json();
    let currentBarang = Array.isArray(db.Barang) ? db.Barang : [];
    let currentKategori = Array.isArray(db.Kategori) ? db.Kategori : [];
    
    // Convert existing to new format ID if needed
    currentBarang = currentBarang.map(b => {
    ...
    const currentBarangIds = new Set(currentBarang.map(b => b.id));
    const currentKategoriIds = new Set(currentKategori.map(k => k.id));
    
    // Merge new items
    let addedBarang = 0;
    newItems.forEach(item => {
      if (!currentBarangIds.has(item.id)) {
        currentBarang.push(item);
        currentBarangIds.add(item.id);
        addedBarang++;
      }
    });
    ...
    categoriesMap.forEach(cat => {
       if (!currentKategoriIds.has(cat.id)) {
          currentKategori.push(cat);
          currentKategoriIds.add(cat.id);
          addedCat++;
       }
    });
    
    db.Barang = currentBarang;
    db.Kategori = currentKategori;
    ...
    if (postRes.ok) {
       console.log(`Successfully added ${addedBarang} new items and ${addedCat} new categories!`);
    } else {
