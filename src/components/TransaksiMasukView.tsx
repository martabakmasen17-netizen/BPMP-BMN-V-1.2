...
export interface CartItemMasuk {
  cartId: string;
  barangId: string;
  namaBarang: string;
  jumlah: number;
...
export default function TransaksiMasukView({
  ...
  // Cart State
  const [cart, setCart] = useState<CartItemMasuk[]>([]);
  ...
  const handleAddToCart = () => {
    ...
    if (existingIndex > -1) {
      ...
      updatedCart[existingIndex].jumlah += jumlah;
      setCart(updatedCart);
    } else {
      setCart([...cart, {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (cart.length === 0) {
      alert("Keranjang kosong! Silakan tambahkan minimal 1 barang.");
      return;
    }
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {cart.map((c) => (
                        <tr key={c.cartId} className="bg-white">
                          ...
                          <td className="py-1.5 px-2 text-right font-bold text-emerald-600">+{c.jumlah} {c.satuan}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {isReadOnly ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6 text-xs font-medium text-gray-700">
                ...
                {cart.length > 0 && (
                  <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-200 space-y-3">
                    <h4 className="font-bold text-emerald-900 text-xs flex items-center gap-2">
                      <Package className="w-4 h-4 text-emerald-700" /> Daftar Barang yang Akan Diterima ({cart.length} item)
                    </h4>
                    <div className="space-y-2">
                      {cart.map((c) => (
                        <div key={c.cartId} className="flex items-center justify-between bg-white p-3 rounded-lg border border-emerald-100 shadow-xs">
                          <div>
                            <p className="font-bold text-gray-900 text-[11px]">{c.namaBarang}</p>
                            <p className="text-[10px] text-gray-500">ID: {c.barangId}</p>
                          </div>
                          <div className="flex items-center gap-4">
                            <span className="font-bold text-emerald-600 text-sm">+{c.jumlah} {c.satuan}</span>
                            <button
                              type="button"
                              onClick={() => handleRemoveFromCart(c.cartId)}
                              className="p-1.5 hover:bg-emerald-100 text-emerald-500 hover:text-emerald-700 rounded-md transition-colors cursor-pointer"
                ...
                <button
                  ...
                  className={`w-full py-3 disabled:opacity-50 text-white rounded-xl text-xs sm:text-sm font-bold shadow-md transition-all cursor-pointer text-center flex items-center justify-center gap-2 ${
                    isLangsungKeluar ? 'bg-blue-600 hover:bg-blue-700' : 'bg-emerald-600 hover:bg-emerald-700'
                  }`}
                >
                  <ArrowDownLeft className="w-4 h-4" />
                  {isLangsungKeluar ? `Checkout & Langsung Distribusikan (${cart.length} Item)` : `Checkout & Proses Pengadaan Masuk (${cart.length} Item)`}
                </button>
              </form>
            )}
          </div>
            ...
            <div className="bg-white border border-amber-300 rounded-2xl shadow-sm overflow-hidden">
              ...
              <div className="p-5 sm:p-7 space-y-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 text-xs font-medium text-gray-700">
                  ...
                  {cart.length > 0 && (
                    <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-200 space-y-3">
                      <h4 className="font-bold text-amber-900 text-xs flex items-center gap-2">
                        <Package className="w-4 h-4 text-amber-700" /> Daftar Barang yang Telah Diterima ({cart.length} item)
                      </h4>
                      <div className="space-y-2">
                        {cart.map((c) => (
                  ...
                  >
                    <CalendarClock className="w-4 h-4" /> Simpan Data Susulan Penerimaan BMN ({cart.length} Item)
                  </button>
                </form>
              </div>
            </div>
